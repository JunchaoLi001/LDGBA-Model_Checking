...
class ControlSynthesis:
    ...
    def __init__(self, pomdp, oa, discount=0.99999, discountB=0.99):
        ...
        num_obsv = 4 
        self.O_matrix = np.zeros((num_obsv, 4, self.num_prod_states)) # O[o, a, s']
        for i,q,r,c in self.states():
            obsv_elements, obsv_probs = self.pomdp.get_observation_prob((r,c))
            for obsv in obsv_elements:
                for q_state in range(self.shape[1]):
                    for action in range(4):
                        self.O_matrix[obsv][action][self.prod_states_idx[i,q,r,c]] += obsv_probs[obsv_elements.index(obsv)]
        self.O_matrix = self.O_matrix/4.
