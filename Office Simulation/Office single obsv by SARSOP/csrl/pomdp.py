...
class GridPOMDP():
    ...
    def s_property(self,row,col,direct):
        ...
        s_property = np.zeros((n_rows, n_cols, 4),dtype = 'int') # s_property (row #, col #, directions('U', 'D', 'R', 'L'))
    ...
    ### office scenario single observation
    def get_observation_prob(self,state):
        """Returns the list of possible observed elements with their probabilities when the current state is the input (obsv_states,obsv_probs).
        The agent observes one of the elements 'hallway'=0, 'wall'=1, 'door'=2 and 'window'=3 on each state;
        (ex. s_property=1,1,2,1 represents ('wall', 'wall', 'door', 'wall'). Agent has probability=0.25 to observe 'door' and probability=0.75 to obseerve 'wall'.)
        ...
        obsv_elements, obsv_probs = [], []
        for direction in range(4):
            ob = self.s_property(state[0],state[1],direction)
            obsv_elements.append(ob)
            obsv_probs.append(1./4)
        
        return obsv_elements, obsv_probs
